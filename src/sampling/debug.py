...
def gaussian_pdf(a, mu, scale=0.5):
...
def plugin_embedding_pi(
    ...
    KT_pi = pairwise_kernels(
        logging_T[:, None], pi_samples[:, None], metric="rbf", gamma=1.0 / (2 * sigmaKT)
    )
...
A_log = gaussian_policy_mean(X_log, beta) + 0.5 * rng.randn(n)
A_tgt = gaussian_policy_mean(X_log, beta) + 2 + 0.5 * rng.randn(n)
pi_fn = lambda a, X: gaussian_pdf(a, gaussian_policy_mean(X_log, beta)+2)
pi0_fn = lambda a, X: gaussian_pdf(a, gaussian_policy_mean(X_log, beta))
A_log = gaussian_policy_mean(X_log, beta) + 0.5 * rng.randn(n)
A_tgt = gaussian_policy_mean(X_log, beta) + 2 + 0.5 * rng.randn(n)
# %%
A_log, A_tgt
...
plot_herding_vs_true(Y_log, Y_tgt, Y_plugin, Y_plugin)
# %%
