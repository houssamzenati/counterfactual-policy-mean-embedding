# Corrected User Size Experiment for Figure 4d

# FAST Mode Settings
num_experiments = 10
user_sizes = [50, 100, 150, 200, 250, 300]
num_cv = 3
...
def simulate_user_size(user_size, config, num_iter):
    results = []

    user_vectors = np.random.normal(0, 1, size=(user_size, config["context_dim"]))
    ...
    for seed in tqdm(seeds, desc=f"User size {user_size}"):
        np.random.seed(seed)
        ...
        for estimator in estimators:
            ...
            results.append(
                {"Estimator": estimator.name, "MSE": mse, "User Size": user_size}
            )
...
full_results = pd.concat(
    [simulate_user_size(u, config, num_experiments) for u in user_sizes]
)

# Plotting results
sns.set(style="whitegrid")
plt.figure(figsize=(8, 5))
sns.lineplot(data=full_results, x="User Size", y="MSE", hue="Estimator", marker="o")
plt.yscale("log")
plt.title("User Size vs MSE (FAST Mode)")
plt.tight_layout()
plt.show()
